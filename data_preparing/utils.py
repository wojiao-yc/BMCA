...
def _ensure_ubp_base_on_path():
    repo_root = os.path.abspath(os.path.join(os.path.dirname(__file__), "..", ".."))
    ubp_root = os.path.join(repo_root, "Uncertainty-aware-Blur-Prior")
    if os.path.isdir(os.path.join(ubp_root, "base")) and ubp_root not in sys.path:
...
def get_obj_from_str(string, reload=False):
    ...
    try:
        ...
        if module.startswith("base") and exc.name == "base":
            _ensure_ubp_base_on_path()
            module_imp = importlib.import_module(module)
        else:
            raise
    if reload:
